<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.tiezifengmian" label="帖子封面" prop="tiezifengmian">
          <file-upload
          tip="点击上传帖子封面"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.tiezifengmian" label="帖子封面" prop="tiezifengmian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.tiezifengmian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.tiezineirong = this.ruleForm.tiezineirong.replace(reg,'../../../ssmru9me/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yingyuanjiaoliu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yingyuanjiaoliuCrossAddOrUpdateFlag = false;
                  this.parent.search();
